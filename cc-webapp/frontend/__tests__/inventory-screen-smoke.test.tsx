...
describe('InventoryScreen - UI smoke (reconciled inventory)', () => {
  test('헤더 총 개수와 리스트가 글로벌 인벤토리를 반영한다', async () => {
    ...
    // 헤더 총 개수 텍스트 확인 (중복 노드 방지: 첫 번째 매칭만 확인)
    const countMatches = await screen.findAllByText(/총 3개 아이템/);
    ...
    expect((await screen.findAllByText(/총 1개 아이템/)).length).toBeGreaterThan(0);

    // 검색 초기화 후 "weapon" 타입 배지가 최소 1개 존재
    fireEvent.change(input, { target: { value: '' } });
    expect((await screen.findAllByText(/총 3개 아이템/)).length).toBeGreaterThan(0);
