...
def upgrade() -> None:
    ...
    # user_rewards: guard for fresh databases where table may not exist yet
    try:
        has_user_rewards = False
        # Inspector.has_table is available on SA 1.4+; fallback to table_names if needed
        if hasattr(insp, 'has_table'):
            has_user_rewards = insp.has_table('user_rewards')  # type: ignore[attr-defined]
        else:
            has_user_rewards = 'user_rewards' in getattr(insp, 'get_table_names')()

        if has_user_rewards:
            ur_cols = {c['name'] for c in insp.get_columns('user_rewards')}
            if 'reward_id' in ur_cols:
                op.alter_column('user_rewards', 'reward_id',
                           existing_type=sa.INTEGER(),
                           nullable=True)
            existing_ur_indexes = {i['name'] for i in insp.get_indexes('user_rewards')}
            if 'claimed_at' in ur_cols and 'ix_user_rewards_claimed_at' not in existing_ur_indexes:
                op.create_index(op.f('ix_user_rewards_claimed_at'), 'user_rewards', ['claimed_at'], unique=False)
        # else: table not present yet → skip safely
    except Exception:
        # Be conservative in non-destructive alignment; skip on unexpected inspector errors
        pass
...
def downgrade() -> None:
    ...
    op.alter_column('user_segments', 'last_updated',
               ...
               nullable=False,
               existing_server_default=sa.text('now()'))
    # Guard user_rewards operations for environments where the table may be absent
    try:
        has_user_rewards = False
        if hasattr(insp, 'has_table'):
            has_user_rewards = insp.has_table('user_rewards')  # type: ignore[attr-defined]
        ...
        if has_user_rewards:
            existing_ur_indexes = {i['name'] for i in insp.get_indexes('user_rewards')}
            # index name may be auto-formatted by op.f(...)
            if op.f('ix_user_rewards_claimed_at') in existing_ur_indexes or 'ix_user_rewards_claimed_at' in existing_ur_indexes:
