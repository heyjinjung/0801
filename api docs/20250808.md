### 2025-08-27 – 게임 컴포넌트 표준화(슬롯/가챠/RPS/크래시)

- 변경 요약
	- NeonSlotGame을 서버 권위로 전환: 로컬 시뮬레이션/가산 로직 제거, 서버 응답 기반 처리, 실패 시 에러 배너와 재시도 버튼 제공. 성공 응답에 balance 포함 시 `mergeProfile`, 미포함 시 최종 `GET /api/users/balance`로 재동기화.
	- RockPaperScissors/NeonCrash/GachaSystem은 `withReconcile`/`withIdem` 패턴으로 멱등키 자동 주입(unifiedApi) + 쓰기 이후 최종 재동기화 적용. 모든 UI는 selector-only로 잔액/통계 읽기(직접 가감 금지).
	- RealtimeSyncProvider의 `reward_granted` 즉시 반영 + TTL 중복 억제와 합쳐, 즉시 UI 반영 후 서버 값으로 일치시키는 흐름 일관화.

- 검증
	- 타입/린트 오류 0. 수동 스모크에서 슬롯/가챠/RPS/크래시 액션 수행 후 UI 잔액이 `/api/users/balance`와 일치 확인. `reward_granted` 수신 시 즉시 증가 후 최종 재동기화로 서버 값과 일치.

- 다음 단계
	- 컨테이너 Playwright 러너로 E2E 재실행(게임 4종 + WS DOM 스모크). 실패 시 GOLD DOM 셀렉터 정밀화 및 초기 하이드레이트 대기(200~300ms) 보정.
	- 스키마 영향 없음 예상. 변동 발생 시 컨테이너 내부에서 OpenAPI 재수출(`python -m app.export_openapi`) 후 본 문서 갱신.
	- 게임별 스탯 패널에 서버 결과 기반 누적 지표 반영 테스트 케이스 추가.
...
### 2025-08-27 – 보상 WS 즉시 반영 + 권위 재동기화

- 변경 요약
	- RealtimeSyncProvider에서 `reward_granted` 이벤트 수신 시, payload 내 `awarded_gold/gold/amount`가 존재하고 `idempotency_reused`가 아닌 경우 GlobalStore에 `applyReward()`를 즉시 호출하여 UI에 골드를 반영합니다.
	- 직후 `hydrateProfile()`을 호출해 서버 권위값으로 최종 일치시킵니다. 1.5초 TTL 기반 중복 억제 로직으로 동일 이벤트 재수신 시 중복 가산을 방지합니다.

- 검증
	- 타입/린트 에러 없음. 동일 세션에서 보상 분배(예: `/api/rewards/distribute`) 후 `reward_granted`와 `profile_update`가 순차 수신되고, UI GOLD가 즉시 증가한 뒤 최종적으로 서버 값과 일치함을 수동 확인.

- 다음 단계
	- 레벨/누적 보상 엔드포인트 확정 시 동일 패턴(POST + withReconcile + applyReward) 적용.
	- E2E 스모크에 `reward_granted` 수신 → UI 즉시 반영 검증 케이스 추가.

### 2025-08-27 – 상점 거래 히스토리: 서버 병합 + 페이지네이션

- 변경 요약
	- `ShopPurchaseHistory`가 서버 거래 히스토리(`GET /api/shop/transactions`) 첫 페이지를 로드하여 WS 기반 최근 구매 목록과 병합합니다. receipt → id → (product_id+timestamp) 우선순위 키로 중복 제거, 최신 timestamp 기준 정렬. 응답이 `{ items, next_cursor }` 또는 배열 형태 모두 지원하며, `next_cursor`가 존재하면 “더 보기” 버튼으로 다음 페이지를 가져옵니다.

- 검증
	- 타입/빌드 에러 없음. 서버가 배열 또는 페이징 객체를 반환해도 UI는 안정적으로 목록을 표시합니다. WS로 신규 구매가 도착하면 병합 로직이 최신 항목으로 덮어씁니다.

- 다음 단계
	- 응답 스키마가 고정되면 전용 타입으로 강화하고 커서 전달 방식(쿼리 vs 헤더)을 문서에 명시합니다. 상태 필터(성공/실패/보류)와 CSV 내보내기 옵션을 추가 검토합니다.
