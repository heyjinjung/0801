### 2025-08-13 – 운영 자동화(Stage 실행/pytest 마커)

- 변경 요약:
	- cc-manage.ps1에 stage 서브커맨드 추가: env/auth/rewards-games/product/hardening/observability/backlog
	- 백엔드 컨테이너 내부 실행 표준화: alembic heads/upgrade, OpenAPI 재수출, pytest 부분 실행
	- 마커 기반 부분 테스트 실행 추가 및 폴백: e2e_* 미태깅 시 auth/game/mvp/integration로 자동 폴백
	- pytest.ini에 e2e_* 마커 선언: e2e_env, e2e_auth, e2e_rewards_games, e2e_product, e2e_ops, e2e_obs, e2e_backlog

- 검증 결과:
	- stage env: Compose 기동→헬스체크→alembic 업그레이드→OpenAPI 재수출 경로 정상 작동
	- stage auth: 상위 `tests/` 수집에서 다수 ImportError가 발생하여 백엔드 pytest.ini의 testpaths를 `app/tests`로 축소. Stage 스크립트는 마커 실패 시 순차 폴백(`app/tests`, `-k auth`, 최종 스모크 `app/tests/test_invite_signup_login_me.py`)하도록 보강.
		- 결과: auth 관련 12개 스모크 테스트 PASS (24 deselected), 수집 오류 0
	- stage rewards-games: e2e_* 마커 미존재 환경에서 기존 game/mvp 마커로 폴백 동작 확인
	- 코드 변경 없음(스크립트/pytest 설정만), 테스트 수집(strict-markers) 경고 없이 통과 기대

- 다음 단계:
	- 백엔드 테스트에 e2e_* 마커 점진 태깅 및 Stage3~5 커버리지 확장
	- 필요 시 OpenAPI 파일(cc-webapp/backend/current_openapi.json) 동기화 훅 보강
	- Stage4 성능/보안 툴 연동(artillery/locust/bandit 등) 검토

### 2025-08-13 – 최종 체크리스트 확장(마스터 E2E/풀스택/배포 전)

- 변경 요약:
	- `20250729-가이드006.md`에 마스터 체크리스트 섹션(7.x) 추가: 전체 유저 플로우, 백엔드/프론트 연동, 데이터/마이그레이션/시드, 성능/부하/동시성, 보안/RBAC, 관측/로깅/알림, 문서/OpenAPI/테스트, 배포 전 최종 점검, 롤백/DR, 역할별 사인오프.
	- 기존 보상 지급 체크리스트(POST /api/rewards) 유지 및 상위 흐름과 연결 명시.

- 검증:
	- 문서 빌드/렌더링 문제 없음(마크다운 체크박스/목록 정상 표시).
	- 중복/충돌된 섹션명 없음, 번호 체계 7.x로 일관 유지.

- 다음 단계:
	- 체크리스트 기반으로 주간 런북 점검 항목 자동화 후보 도출(PS 스크립트/pytest 태그 연계).
	- OpenAPI 재수출 필요 시 문서 내 절차(컨테이너 내부 python -m app.export_openapi) 재검증.
	- CI에 pre-flight 체크(환경변수/헬스/알림 WS 연결) 스텝 추가 검토.

### 2025-08-13 – 기능별 세부 체크리스트 추가(구현/미구현 표기)

- 변경 요약:
	- `20250729-가이드006.md`에 '8) 기능별 세부 체크리스트' 신설. 도메인(인증/게임/보상/상점/배틀패스/이벤트·미션/개인화/성인/알림/Admin/데이터/Redis/Kafka/관측/프론트/OBS)별로 ✅/⏳/TBD 표기.

- 검증:
	- 문서 마크다운 렌더링 확인, 중복 섹션명 없음, 상위 7.x 마스터 체크리스트와 보완 관계 유지.

- 다음 단계:
	- TBD 항목에 대한 오너/우선순위 지정 후 스프린트 백로그 편입.
	- CI 보고서에 섹션별 체크 항목 자동 집계(예: pytest 마커/커버리지 매핑) 검토.

### 2025-08-13 – 프로덕션 하드닝 체크리스트 추가

- 변경 요약:
	- `20250729-가이드006.md`에 '9) 프로덕션 하드닝 체크리스트' 신설(보안/배포/DB/Redis/Kafka/성능/관측/개인정보/DR 등).

- 검증:
	- 문서 렌더링 정상, 상위 체크리스트와 중복 없이 보완적 구성 확인.

- 다음 단계:
	- SLO·알림 룰을 실제 대시보드와 연결, 런북/DR 리허설 일정화.
	- compose.prod 리소스 제한/헬스체크 값 최적화 및 비용 목표 수립.

### 2025-08-13 – 스코프 조정 및 실행 우선순위 추가

- 변경 요약:
	- 배틀패스 유료 트랙: 본 릴리스에서 삭제(문서 8.5에 ⏸/삭제 표기).
	- 개인화(8.7), 성인 컨텐츠(8.8) 전면 보류(⏸), 차기 패치로 이월.
	- 10) 우선순위 실행 순서 섹션 추가: 필수 안정화→제품 마감→운영 하드닝→보류 항목.

- 검증:
	- 문서 번호/링크 충돌 없음, 체크 상태 범례에 ‘⏸ 보류’ 추가 확인.

- 다음 단계:
	- 스프린트 계획에 우선순위 1,2 항목 구체 태스크 분해 및 담당 지정.
	- 보류 항목은 후속 릴리스 노트/로드맵에 이동.

### 2025-08-13 – 실행 순서(검증/구현/연동) 섹션 추가

- 변경 요약:
	- `20250729-가이드006.md` 말미에 "11) 실행 순서 — 검증/구현/연동" 추가. 단계 0~6로 환경→인증/유저→보상/게임→제품 마감→운영 하드닝→관측/런북→보류 관리 순서 명시.

- 검증:
	- 문서 렌더링 및 번호 연속성 확인, 상위 우선순위/프로덕션 섹션과 정합.

- 다음 단계:
	- 단계별 자동 점검 스크립트화(docker-manage.ps1 서브커맨드/pytest 마커) 검토.
