...
- 변경 요약
	...
	- `components/admin/ShopManager.tsx` 는 데모/미사용 표기로 정리(중복 방지, AdminPanel로 통합 유지).
...
- 변경 요약
	...
	- 루트 페이지에 비간섭 프로필 셸(프로필 통계/최근 활동) 오버레이 추가(셀렉터 안정성 향상). `/profile` 경로는 기존 쉘 유지.
		- 중복 제거 진행: `components/admin/ShopManager.tsx`를 빌드 영향 없도록 무해화(빈 모듈) 처리했고, AdminPanel로 기능을 일원화. 실제 파일 삭제는 빌드 캐시 경로 정리 후 수행 예정.
