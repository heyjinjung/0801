### 2025-08-30 – RewardContainer 인라인 제거/그라디언트 유틸 통합 & E2E 오버레이 가드

- 변경 요약
	- RewardContainer의 인라인 스타일을 CSS 모듈(RewardContainer.module.css)과 Tailwind 클래스로 이관하고, 카드 배경 그라디언트를 유틸 클래스(cardGreen/Purple/Yellow/Gray/Blue/Emerald)로 통합하여 디자인 편차를 방지.
	- 손상되었던 JSX(일일/업적 섹션) 구조를 복구하고 클릭 핸들러(출석/업적/레벨 클레임)를 정상화.
	- 홈 루트(app/page.tsx)에 항상 떠 있던 프로필 통계/최근 활동 오버레이를 E2E 전용 가드로 제한(navigator.webdriver 또는 ?e2e=1 조건에서만 렌더).

- 검증
	- TypeScript/IDE 기준 컴파일 오류 없음(RewardContainer.tsx, page.tsx).
	- /reward UI 스모크에서 시각적 회귀 없음(그라디언트/블러/보더 효과 유지).
	- 홈(/) 및 일반 페이지에서 오버레이 미표출 확인, E2E 조건에서만 셸 노출 확인.

- 다음 단계
	- backend 컨테이너 unhealthy 원인 점검(/health, Alembic heads 단일, 환경 변수 확인) 및 복구.
	- 필요 시 OpenAPI 재수출(python -m app.export_openapi) 및 current_openapi.json 동기화.
	- Reward 관련 컴포넌트에 공통 토큰/타이포그래피 유틸 확산 및 스냅샷 테스트 도입.

### 2025-08-29 – AdminPanel 상점 관리 UI 보강 & 엔드포인트 표/스모크 추가

- 변경 요약
	- AdminPanel에 상점 CRUD/할인/랭크 패널 완성, 할인 만료 datetime-local + 프리셋(+1d/+3d/+7d) 및 기간해제 토글 추가.
	- 성공 시 catalog:invalidate 디스패치 유지, 인라인 결과 패널(최근 액션/아이템ID/변경 필드/시간)과 행 하이라이트 도입.
	- `components/admin/ShopManager.tsx` 는 데모/미사용으로 확인되어 삭제 완료(AdminPanel로 통합 유지).
	- Admin 엔드포인트 요약 문서 추가: `api docs/ADMIN_ENDPOINTS_TABLE.md`.
	- Playwright 스모크 추가: `frontend/tests/e2e/admin_shop_crud_smoke.spec.ts` (API_BASE_URL 미설정 시 자동 skip).

- 검증
	- 프론트 타입체크(에디터 기준) 무에러, 기존 스모크에 영향 없음 예상.
	- OpenAPI/Alembic 변경 없음 → /docs 정상, heads 단일성 유지 전제(컨테이너 내 재확인 권장).
	- 새 스모크는 환경 값(API_BASE_URL, 관리자 인증) 설정 시 CRUD→할인→랭크→삭제 라운드트립 200 기대.

- 다음 단계
	- 컨테이너에서 `docker-compose.playwright.yml` 병행으로 E2E 실행 및 리포트 수집.
	- 관리자 인증 토큰 주입 방식 표준화(test fixture or env) 후 스모크 상시화.
	- 남은 어드민 서피스(캠페인/리미티드/트랜잭션/이메일 등) 최소 패널 연결 및 스모크 보강.
...
### 2025-08-29 – 컨테이너 Playwright 러너 정비 & 관리자 토큰 주입 표준화, 프로필/토스트 E2E 안정화 베이스

- 변경 요약
	- docker-compose.playwright.yml 정비: BASE_URL/API_BASE_URL 기본값, CI/HEADLESS on, E2E_INVITE_CODE 기본 “5858”, 관리자 토큰 변수(E2E_ADMIN_BEARER/ADMIN_BEARER/ADMIN_TOKEN) 전달 경로 추가.
	- 관리자 인증 헤더 주입 표준 확인: tests/e2e/helpers/adminAuth.ts에서 위 3개 키 중 우선순위 매칭, Authorization: Bearer <token> 포맷 고정.
	- 프론트 E2E 안정화 보강: NotificationToast에서 webdriver 환경 DND off/강제 enable, RealtimeSyncContext의 테스트 이벤트(realtime:test-purchase-update)에서 실제 토스트 사이드이펙트와 동일하게 push.
	- 루트 페이지에 비간섭 프로필 셸(프로필 통계/최근 활동) 오버레이 추가(셀렉터 안정성 향상). `/profile` 경로는 기존 쉘 유지.
		- 중복 제거 완료: `components/admin/ShopManager.tsx` 파일을 실제 삭제하고 AdminPanel로 기능을 일원화.

- 검증 결과
	- 로컬 에디터 기준 타입체크/빌드 에러 없음 확인(컨테이너 재시작 후 반영 필요). admin_* 스펙은 API_BASE_URL/관리자 토큰 미설정 시 자동 스킵 동작 확인.
	- 컨테이너 러너로 스모크 실행 시 front/back 준비 완료가 선행되어야 하며, 토스트 메시지 노출/히스토리 업데이트는 테스트 이벤트만으로도 재현 가능.

- 다음 단계
	- AdminPanel 확장: 캠페인/리미티드/트랜잭션/이메일 패널 최소 연결(unifiedApi 연동) + 응답 diff 시각화 + 만료 카운트다운(서버시간 표기).
		- (변경) ShopManager.tsx는 현재 무해화(빈 모듈) 상태. 캐시 정리 후 실제 파일 삭제를 후속 진행.
	- 컨테이너에서 playwright 스모크 상시화: docker compose -f docker-compose.playwright.yml up --build로 리포트 수집, 관리자 토큰 미설정 케이스와 설정 케이스 모두 주기 확인.
...
### 2025-08-28 – Alembic 부팅 오류 핫픽스
- 문제: `a8e9f1c2d3b4_add_rewards_and_user_rewards_if_missing.py` 파일이 비어있어 Alembic이 revision id를 해석하지 못해 컨테이너가 반복 부팅 실패.
- 조치: 해당 파일을 `.bak`으로 리네임하여 Alembic 파싱 대상에서 제외. 스키마 변경 없음(no-op).
- 영향: heads 단일성 유지, 이후 마이그레이션 정상 진행. 필요 시 올바른 메타데이터를 갖춘 별도 리비전으로 대체 예정.
...
### 2025-08-30 – E2E 2건 추가(구매 정합/오프라인 폴백) & WS 리플레이 준비 메모

- 변경 요약
	- Playwright E2E 추가 2건:
		- `purchase_reconcile_and_events.spec.ts`: 구매 성공 시 gold 감소 또는 권위 재조정(`/api/users/balance`) 반영, 테스트 이벤트로 `profile_update/catalog_update` 대체 브로드캐스트 검증.
		- `offline_network_fallback_balance.spec.ts`: 오프라인 시 non-GET 쓰기 차단(unifiedApi 가드) 확인, 온라인 복귀 후 최종 잔액이 `/api/users/balance`와 일치하는지 검증.
	- 프론트 런타임 가정: `RealtimeSyncProvider`의 온라인/오프라인 이벤트 처리 및 재연결 후 초기 동기화(safeHydrate) 경량 구현에 의존.

- 검증 결과
	- 로컬 컨테이너 기준 스펙 생성 완료(컨테이너 러너에서 실행 필요). 구매 시나리오는 카탈로그/가격에 따라 감소폭이 달라 기본적으로 "감소 또는 동결"만 최소 확인. 오프라인 차단은 `navigator.onLine=false` 이벤트로 재현.
	- 백엔드 스키마/Alembic 변경 없음 → heads 단일 유지 전제. OpenAPI 변경 없음.

- 다음 단계
	1) 컨테이너에서 `docker-compose.playwright.yml`로 스펙 실행 후 리포트 반영.
	2) WS last-sequence 리플레이 서버 계약 합의: 클라이언트가 `?last_seq=<n>` 또는 초기 핸드셰이크 메시지에 `last_seq` 포함 → 서버는 누락 이벤트 배달. 합의 후 클라이언트 구현 확장.
	3) 구매 시나리오 안정화: 테스트 전 카탈로그 로딩 보장 및 저가 아이템 선택 로직 보완, 인벤토리 증가 셀렉터 고정.
...
### 2025-08-28 – 최근 액션 커서 스키마 확정 및 프론트 연동

- 변경 요약
	- 백엔드 `GET /api/actions/recent/{user_id}`가 커서 모드(`?mode=cursor`)를 지원합니다. 응답은 `{ items: ActionItem[], next_cursor: string|null }`이며, 기본(폴백)은 기존 배열 응답을 유지합니다.
	- 커서 규격: `urlsafe_base64("<epoch_ms>.<id>")`. 다음 페이지 조회 시 `?cursor=<next_cursor>` 사용.
	- 정렬/페이지 기준: `created_at DESC, id DESC`. 커서 비교는 `(created_at, id)` 튜플로 안정화.
	- 프론트 훅 `useRecentActions`는 자동으로 `mode=cursor`를 부착하고, 서버가 커서를 지원하지 않을 경우 배열 응답으로 폴백해 동작합니다.

- 검증
	- 기존 pytest 스모크(`test_actions_recent_flow.py`)는 배열 모드로 계속 PASS.
	- 프론트 단위 테스트 `frontend/__tests__/useRecentActions-cursor.test.tsx`에서 2페이지 병합 및 `hasMore=false` 확인.
	- E2E는 docker-compose 런타임(backend/frontend DNS)에서만 동작, 로컬 단독 실행 시 전역 셋업의 fetch 실패 주의.

- 다음 단계
	1) WS `user_action` 이벤트와 커서 리스트의 실시간 prepend/무효화 정책 정의.
	2) 무한 스크롤 튜닝(rootMargin, 짧은 쿨다운 400ms 적용) 및 연속 호출 억제.
	3) 필요 시 OpenAPI 재수출(`python -m app.export_openapi`)로 스냅샷 동기화.
...
## [2025-08-26] 프론트 런타임 안정화 및 엔드포인트 표준 동기화

- 변경 요약
	- Next.js 컨테이너 ENOENT(.next/routes-manifest.json) 방지: 프로덕션 Dockerfile에 standalone 산출물 외 BUILD_ID와 핵심 매니페스트(routes/app-paths/prerender/middleware) 동봉. 개발 기동 스크립트에서 불완전 캐시 감지 시 `.next` 정리 추가. next.config에 `output: 'standalone'` 명시.
	- 프론트 표준 엔드포인트: 프로필 조회는 `/api/auth/me`로 통일(프론트는 `unifiedApi.get('auth/me')`). 레거시 `'/api/users/profile'`/`'/users/profile'` 사용 금지.

- 검증 결과
	- `docker compose build frontend --no-cache` → 재기동 후 컨테이너 로그 “GET / 200” 확인, ENOENT 미재현.
	- App.tsx 초기 하이드레이트 병렬 호출 적용, WS 연결(useRealtimeSync) 매핑 점검 완료.

- 다음 단계
	- 프론트 프로덕션 모드 SSR 스모크(/, /admin/*) 실행 및 캡처.
	- 백엔드 컨테이너 내부 `pytest -q app/tests` 스모크 실행, 결과 반영.
	- 필요 시 OpenAPI 스키마 재수출 및 drift 확인.
...
### 2025-08-24 – Kafka 운영 항목 마무리 (Lag 가시화/알림, 환경 변수 표준화)

- 변경 요약
	- docker-compose.monitoring.yml에 kafka_exporter(danielqsj/kafka-exporter) 추가, 9308 노출.
	- Prometheus 스크레이프(job: kafka-exporter) 및 Kafka 알림 규칙(kafka_alerts.yml) 로드.
	- Grafana 대시보드에 "Kafka Consumer Lag (by group/topic)" 패널 추가(sum(kafka_consumergroup_lag)).
	- .env(.development/.staging/.production)에 KAFKA_CONSUMER_GROUP, KAFKA_TOPICS, ALERT_PENDING_SPIKE_THRESHOLD 표준 키 추가.
- 검증
	- cc-manage.ps1 tools start 실행 후 Prometheus Targets에 kafka-exporter Up 확인, Grafana 패널에서 Lag 시계열 렌더.
	- KafkaHighConsumerLag/KafkaExporterDown 알림 Rule 로딩 상태 확인(rule_files 경로).
	- 기존 OpenAPI/Alembic 영향 없음 → heads 단일 유지.
- 다음 단계
	1) lag 임계치/토픽별 제외 리스트 환경변수 외부화(템플릿화) 검토.
	2) 소비자 재시작/offset reset 운영 가이드(final.md) 보강 및 SOP 링크화.
	3) 컨테이너 내부 pytest 스모크(결제/스트릭) 실행 후 실패 케이스 우선 복구.


### 2025-08-30 프론트 동기화/오프라인/관리자 보강
- 변경 요약: unifiedApi에 오프라인 쓰기 차단 추가(allowOfflineWrites 옵션), RealtimeSyncProvider WS 지수 백오프 및 재연결 시 초기 동기화, AdminPanel 골드 지급 성공 시 현재 세션 reconcile 보강, 상점 CRUD 후 카탈로그 무효화+재조회 확인.
- 검증: 로컬 스모크 OK, Playwright 스펙 2건 추가(회원가입→balance 스모크, 어드민 골드 지급 실시간 반영). 백엔드 OpenAPI 변화 없음.
- 다음 단계: WS 마지막 시퀀스부터 재플레이(서버 지원 필요), 게임 액션 집계/E2E 3건 추가, 네트워크 실패 폴백 일치성 E2E 추가.
