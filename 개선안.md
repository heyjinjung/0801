...
- `components/admin/ShopManager.tsx`는 빌드 영향 방지를 위해 무해화(빈 모듈) 처리했으며, 기능은 AdminPanel로 일원화됨. 실제 파일 삭제는 캐시 정리 후 진행 예정.
...
검증 결과
- ShopManager.tsx는 현재 무해화(빈 모듈) 상태(외부 참조 없음 확인). AdminPanel 확장으로 대체.
...
- 변경 요약
  ...
  - 중복 제거: components/admin/ShopManager.tsx 무해화(빈 모듈) 처리. AdminPanel로 기능 일원화(기능 측면 완료, 실제 파일 삭제는 후속).
