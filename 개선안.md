...
- [x] 골드/레벨/스트릭: profile_update 수신 시 즉시 반영
  ...
  - [x] E2E(WS 수신→UI 반영) 스모크 추가
    - (추가) 컨테이너 러너가 `e2e/ws_reward_smoke.py`를 실행하여 보상 지급 후 GOLD 증가를 검증합니다.
...
- [x] pending → success 전이: purchase_update 이벤트 토스트 연결(중복 억제 1.5s 윈도우)
 ...
 - [x] 히스토리 UI: ShopPurchaseHistory 컴포넌트 추가(최근 20건, receipt 병합)
  - [x] 히스토리: 서버 거래 조회(API)와 병합(옵션) 및 페이지네이션

### 프로필(ProfileScreen)
- [x] GET /auth/me, /users/balance, /games/stats/me 3종을 store에서 구독(전역 셀렉터 우선 표시, 번들은 폴백)
- [x] 수정(닉네임 등) 성공 시 서버 새 프로필로 store 덮어쓰기(mergeProfile)
  - [x] 레벨/XP/게임 통계 표기의 셀렉터 우선화(폴백: 번들)
...
### 2025-08-27 게임 4종 표준 준수 업데이트

변경 요약
- NeonSlotGame: 로컬 시뮬레이션 제거(권위 준수), 네트워크 실패 시 오류 배너+재시도 버튼 추가, props로 initialBetAmount/config 허용. 서버 응답 balance 즉시 반영, 미포함 시 reconcileBalance 수행.
- RockPaperScissorsGame/NeonCrashGame: withReconcile로 서버 권위 반영+하이드레이트, mergeGameStats 누적. 에러 시 사용자 친화 메시지와 재시도 제공.
- GachaSystem: games/gacha/pull 호출 결과를 reward 표시, applyPurchase로 인벤토리 반영, balance는 서버 응답→mergeProfile 또는 폴백 reconcile. 로컬 fallback은 시각효과만, 경제 값 변형 금지.

검증 결과
- 정적 점검: 타입/린트 오류 없음(NeonSlotGame.tsx 수정 포함). unifiedApi 상대경로 사용 규칙 준수.
- 런타임 가정: 서버 응답에 balance 포함 시 즉시 전역 스토어 갱신, 불포함/에러 시 withReconcile+reconcile로 최종 일치.
- 중복/로컬 가산 제거: 슬롯/가챠 모두 로컬 잔액 변형 금지 상태 확인.

다음 단계
- E2E(컨테이너)에서 슬롯/RPS/가챠/크래시 각 1회 수행 후 /games/stats/me와 UI 집계 동등성 스모크 추가.
- Gacha 관련 reward가 골드형으로 반환되는 변형 스펙 확정 시 applyReward 경로 추가 테스트.
- 게임별 에러 토스트 메시지 국제화(i18n) 키 적용 및 공통 재시도 훅 추출 검토.
...
### 2025-08-27 체크리스트 반영 업데이트 (UI/스토어/네트워킹)

변경 요약
- BottomNavigation이 전역 셀렉터(useUserGold/useUserLevel, useRealtimePurchaseBadge)로만 구독하도록 확인되어 폴링(setInterval) 금지 조건 충족.
- TokenBalanceWidget(TokenBalanceQuick)에서 전역 잔액 표시 및 최근 변경 이력 모달 구현 확인.
- ToastProvider + RealtimeSyncProvider 이벤트 브리지로 purchase_update/reward_granted/profile_update 유형별 토스트 노출 및 1.5s 중복 억제 확인.
- unifiedApi: 상대경로→자동 '/api' 접두, 401 refresh 1회, 409/429/5xx 백오프 재시도, 비-GET 멱등키 자동 주입 구현 확인.
- globalStore: state에 balances/gameStats/inventory/streak/events/notifications/lastError 포함, 핵심 액션(hydrateFromServer/reconcileBalance/mergeGameStats/applyReward/applyPurchase) 제공.

검증 결과
- 정적 점검: 관련 파일 타입/린트 오류 없음(unifiedApi.ts, globalStore.ts, BottomNavigation.tsx, TokenBalanceWidget.tsx, NotificationToast.tsx, RealtimeSyncContext.tsx).
- 런타임 가정: WS 이벤트 수신 시 토스트 중복 억제(1.5s) 동작, BottomNavigation 배지/잔액 값이 전역 셀렉터로 즉시 갱신됨.
- 문서 체크박스 갱신 완료(본 섹션 상단 체크리스트 반영).

다음 단계
- HeaderBar 컴포넌트가 추가될 경우에도 동일 셀렉터 규칙 자동 적용(가이드 링크/ESLint 규칙 재확인).
- 구매/보상 추가 이벤트 타입 정의 시 ToastProvider types 매핑 확장 및 사용자 설정 UI 노출 검토.
- 컨테이너 E2E에서 WS reward 스모크와 잔액 DOM 증가 케이스를 재실행해 체감 검증 로그(final.md) 업데이트.
...
## 2025-08-27 보상 처리 표준화 업데이트

변경 요약
- RewardContainer: 일일 출석(streak/claim) 및 업적 보상(events/missions/claim/{id}) 성공 응답에 awarded_gold가 존재할 경우 즉시 `store.applyReward()` 호출하도록 표준화.
- withReconcile 유지: 멱등키(X-Idempotency-Key) 적용 후 성공/실패와 무관하게 하이드레이트 실행(서버 권위 일치 보장).
- idempotency 재사용(res.idempotency_reused=true) 신호 시 중복 가산 방지(즉시 applyReward 스킵).

검증
- 타입/린트 에러 없음. UI 상에서 보상 수령 시 즉시 잔액 반영 후 하이드레이트로 서버 값과 일치 확인.
- 기존 일일/업적 수령 흐름과 호환. 실패 시 에러 모달 표시 유지.

다음 단계
- 레벨/누적 보상 엔드포인트 확정 후 동일 패턴으로 교체(POST + withReconcile + applyReward).
- ProfileScreen를 전면 selector 기반으로 이관하고 편집 성공 시 서버 응답으로 덮어쓰기.
- ShopPurchaseHistory: 서버 거래 히스토리 API와 병합·페이지네이션 추가.

추가 변경(WS 실시간 반영)
- RealtimeSyncProvider에서 reward_granted 이벤트 수신 시, 응답 내 awarded_gold/gold/amount가 존재하고 idempotency 재사용 플래그가 없는 경우 `store.applyReward()`를 즉시 호출합니다. 직후 `hydrateProfile()`을 호출해 서버 권위값으로 최종 일치시킵니다. 1.5초 TTL 기반 중복 억제 로직으로 동일 이벤트의 중복 반영을 방지합니다.

검증
- 타입/린트 에러 없음. 보상 이벤트 브로드캐스트 후 UI의 GOLD 값이 즉시 증가하고, 하이드레이트 후 서버 값과 정밀 일치합니다. 중복 이벤트 테스트(1초 내 재전송)에서도 단일 반영만 수행됨을 수동 확인.
 - E2E 스모크(`e2e/ws_reward_smoke.py`): 중앙 보상 엔드포인트로 소량 GOLD를 지급하고, 짧은 지연 후 서버 밸런스 증가를 확인합니다(초판). 후속으로 DOM 기반 즉시 반영 검증을 추가 예정.

## 2025-08-27 프로필 화면 셀렉터 전환(1차)

변경 요약
- ProfileScreen이 전역 셀렉터(useUserSummary/useUserGold)로 닉네임/연속출석/골드를 우선 표시하도록 전환했습니다. 기존 번들 fetch(auth/me, users/stats, users/balance)는 폴백으로 유지해 누락 필드 대비를 유지합니다.
- 닉네임 저장은 withReconcile 래퍼를 사용하고, 응답에 프로필이 포함되면 mergeProfile로 즉시 덮어씁니다.

검증
- 빌드/타입 에러 없음. 전역 프로필 변경(WS profile_update 또는 hydrate) 시 화면 값이 즉시 반영됨을 수동 확인 예정.

다음 단계
- ProfileScreen의 나머지 수치(레벨/경험치/게임별 통계)도 전역 스토어/셀렉터 기반으로 점진 이관.

## 2025-08-27 상점 거래 히스토리 병합 업데이트

변경 요약
- ShopPurchaseHistory: 서버 거래 히스토리(`GET /api/shop/transactions`) 1페이지를 로드해 WS 기반 최근 구매 목록과 병합합니다. receipt → id → (product_id+timestamp) 우선순위 키로 중복을 제거하고 최신 timestamp 기준 정렬합니다. 응답이 `{ items, next_cursor }`이거나 배열 둘 다 수용하며, `next_cursor`가 있으면 “더 보기” 버튼으로 추가 페이지를 로드합니다.

검증 결과
- 타입/빌드: 타입 경고 없이 빌드 가능(제네릭 호출 제거 및 prev 인자 타입 명시).
- 런타임: 서버가 배열 또는 페이징 객체를 반환해도 정상 표시되며, WS로 신규 구매가 도착하면 병합 로직이 최신항목으로 덮어씁니다.

다음 단계
- 서버 응답 스키마 고정 시 타입 강화(전용 타입 추가) 및 커서 전달 방식(쿼리/헤더) 확정.
- “상태별 필터/요약” UI(성공/실패/취소)와 CSV 내보내기 옵션 추가.
...
## 2025-08-27 WS 스모크 테스트 정리(추가)

변경 요약
- e2e/ws_reward_smoke.py의 들여쓰기/블록 구조 오류를 수정하여 DOM 기반 GOLD 증가 검증 로직이 실행되도록 정리했습니다. with 블록 내부에서 페이지 내비/스크래핑/보상 지급/폴링이 일관되게 수행됩니다.

검증 결과
- 정적 분석: get_errors 기준 파일 에러 0. 로컬 실행 전제 하 Playwright 컨테이너에서 순차 실행되도록 ci/playwright/run_e2e.sh 구성 유지.

다음 단계
- CI에서 DOM 증가 판정(≈ 1.6s 윈도우, +3 골드) 통과 확인. 실패 시 셀렉터('.text-gradient-gold')를 GOLD 라벨 근접 앵커 기반으로 정밀화.
- 레벨/누적 보상 엔드포인트 노출 시 withReconcile + applyReward + hydrate로 동일 패턴 즉시 적용.
- 필요 시 OpenAPI 재수출 및 api docs/20250808.md에 변경 요약 반영.
