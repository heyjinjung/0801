...
## 2025-08-30: Admin/리얼타임/오프라인/경합 보강 및 E2E 추가

- 변경 요약
  - unifiedApi: 오프라인 시 비-GET 쓰기 요청을 차단(allowOfflineWrites 옵션 추가)하여 경제 액션 큐잉 금지.
  - RealtimeSyncProvider: WS 지수 백오프 재시도 도입, 재연결 직후 초기 sync 트리거(safeHydrate).
  - AdminPanel: 골드 지급 성공 시 현재 세션이면 즉시 reconcileBalance 호출(브로드캐스트 전제 보완 주석).
  - E2E: 회원가입→로그인→/users/balance 스모크, 어드민 골드 지급(테스트 이벤트) 실시간 반영 스펙 추가.

- 검증 결과
  - 프론트 빌드/테스트: 로컬 스모크 기준 OK, Playwright 스펙 2건 추가(컨테이너 환경에서 실행 예정).
  - 런타임: 오프라인 강제 시 쓰기 API 호출이 차단되는지 수동 확인 필요.
  - OpenAPI 영향 없음.

- 다음 단계
  - WS 재연결 시 서버 지원(last sequence replay) 연동 검토.
  - 어드민 상점 업서트 후 카탈로그 캐시 무효화가 클라이언트 전역에 반영되는지 컨테이너 E2E에서 재확인.
  - 네트워크 실패 폴백 시 최종 잔액이 /users/balance와 일치하는지 추가 E2E 작성.

### 2025-08-30 추가: 구매 정합/오프라인 폴백 E2E 도입 + WS 리플레이 준비

- 변경 요약
  - Playwright 스펙 2건 추가:
    - `frontend/tests/e2e/purchase_reconcile_and_events.spec.ts`: 구매 시 gold 감소 또는 권위 재조정(`/api/users/balance`) 반영 확인, 테스트 이벤트로 `realtime:test-profile-update`/`realtime:test-catalog-update` 트리거.
    - `frontend/tests/e2e/offline_network_fallback_balance.spec.ts`: 오프라인(navigator.onLine=false) 상태에서 non-GET 쓰기 차단을 확인하고, 온라인 복귀 후 최종 잔액이 권위(`/api/users/balance`)와 동일함을 검증.
  - 런타임 가정: unifiedApi 오프라인 가드와 RealtimeSyncProvider의 online/offline 핸들링, 재연결 직후 safeHydrate를 전제로 함.

- 검증 결과
  - 스펙 파일 생성 및 기본 시나리오 구성 완료(컨테이너 러너에서 실행 필요). 카탈로그/가격 미확정 환경을 고려하여 “감소 또는 동결” 수준으로 최소 확인.
  - 백엔드 스키마/Alembic 변경 없음, OpenAPI 변경 없음.

- 다음 단계
  1) docker-compose.playwright.yml 러너로 스펙 실행 및 리포트 수집.
  2) WS last-sequence 리플레이 서버 계약 확정 후 클라 측 핸드셰이크(`last_seq`) 구현.
  3) 구매 스펙에서 저가 아이템 고정 선택 또는 테스트용 카탈로그 시드 도입으로 감소 검증을 안정화.
