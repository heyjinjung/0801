...
- `ActionHistory`에 IntersectionObserver로 자동 로드(sentinel) 추가, 역할(role=list/listitem) 부여.
- E2E: `profile_stats_action_history.spec.ts` 추가, `action_history_pagination.spec.ts` 보강, PR-smoke에 프로필 스펙 포함.

검증 결과
- 로컬 단일 파일 타입체크/린트 에러 없음. 컴포넌트/훅 빌드 에러 없음 확인.
- E2E는 글로벌 셋업에서 backend DNS 의존(backend, frontend)로 로컬 단독 실행 시 실패 가능. CI/compose 환경(API_BASE_URL/BASE_URL 설정)에서는 통과 예상.
- 기존 스펙과 충돌 없음(셀렉터 안정화: aria-label, role 추가).
